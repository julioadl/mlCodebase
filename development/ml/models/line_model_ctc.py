...
from datasets import EmnistLinesDataset
from models.base import ModelTf as Model
from algorithms.line_lstm_ctc import line_lstm_ctc


class LineModelCTC(Model):
    def __init__(self, dataset_cls: type=EmnistLinesDataset, algorithm_fn: Callable=line_lstm_ctc, dataset_args: Dict=None, algorithm_args: Dict=None):
        ...
        default_algorithm_args = {'window_width': 14, 'window_stride': 7}
        if algorithm_args is None:
            algorithm_args = {}
        algorithm_args = {**default_algorithm_args, **algorithm_args}
    ...
    def evaluate(self, x, y, batch_size: int=16, verbose=True) -> float:
        ...
        decoding_model = KerasModel(inputs=self.algorithm.input, outputs=self.algorithm.get_layer('ctc_decoded').output)
        preds = decoding_model.predict_generator(test_sequence)
        trues = np.argmax(y, -1)
        pred_strings = [''.join(self.data.mapping.get(label, '') for label in int(pred)).strip(' |_') for pred in preds]
    ...
    def predict_on_image(self, image: np.ndarray) -> Tuple[str, float]:
        softmax_output_fn = K.function(
            [self.algorithm.get_layer('image').input, K.learning_phase()],
            [self.algorithm.get_layer('softmax_output').output]
        )
